...
export const createPostSchemaBase = z.object({
  ...
  // Updated to 2000 characters for longer descriptions
...
export function createPostSchema(tier: 'free' | 'premium' | 'business' = 'free') {
  ...
  const allowedGalleryTypes = tier === 'free' 
    ? ['hover', 'horizontal', 'vertical', 'gallery'] as const
    : tier === 'premium' 
    ? ['hover', 'horizontal', 'vertical', 'gallery', 'before_after', 'video'] as const
    : ['hover', 'horizontal', 'vertical', 'gallery', 'before_after', 'video', 'premium'] as const
