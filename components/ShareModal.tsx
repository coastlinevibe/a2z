...
export function ShareModal({ isOpen, onClose, post, username, className }: ShareModalProps) {
  ...
  // Use the actual username from the profile
  const actualUsername = username || post.username
  ...
  // Use direct username/slug format
  const publicUrl = actualUsername ? `${baseUrl}/${actualUsername}/${post.slug}` : `${baseUrl}/p/${post.slug}`
